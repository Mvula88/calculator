...
interface SimpleContentProtectionProps {
...
export default function SimpleContentProtection({ children, userEmail = 'Portal User' }: SimpleContentProtectionProps) {
  ...
  useEffect(() => {
    ...
    let email = userEmail
    if (!email || email === 'Portal User') {
      // Try to get from session
      const stored = localStorage.getItem('impota_session')
      if (stored) {
        try {
          const session = JSON.parse(stored)
          if (session.email && !session.email.startsWith('user_')) {
            email = session.email
          }
        } catch (e) {}
      }
    ...
    <div className="relative select-none">
      ...
      <div className="fixed inset-0 z-40 pointer-events-none overflow-hidden">
        {Array.from({ length: 30 }).map((_, i) => (
          <div
            key={i}
            className="absolute text-gray-300/20 font-bold text-xl transform rotate-45 whitespace-nowrap"
            style={{
              top: `${(i % 6) * 17}%`,
