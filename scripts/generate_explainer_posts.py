# -*- coding: utf-8 -*-
"""Generate 10 explainer posts: what IMPOTA is, where cars come from,
the journey, what's inside the platform, country playbooks, etc.

These are educational posts (vs. the N$200 conversion posts) — they
introduce a cold audience to the IMPOTA value proposition.
"""
from pathlib import Path

ROOT = Path(r"c:\Users\ismae\Desktop\calculator\public\designs")

TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Playfair+Display:ital,wght@0,400;0,700;0,800;0,900;1,400;1,700;1,900&display=swap" rel="stylesheet">
<title>{title}</title>
<style>
  *{{margin:0;padding:0;box-sizing:border-box}}
  :root{{--green-deep:#09090b;--green-mid:#18181b;--green-bright:#f59e0b;--gold:#fbbf24;--cream:#fafaf9}}
  body{{width:1080px;height:1080px;overflow:hidden;font-family:'Inter',sans-serif;background:#000;color:var(--cream);position:relative}}

  .photo{{position:absolute;inset:0;background:url('/Background%20Images/{bg}') center center / cover no-repeat;filter:saturate(0.96) contrast(1.06) brightness(1.06);z-index:1}}
  .vignette{{position:absolute;inset:0;background:radial-gradient(ellipse 95% 95% at 60% 50%, rgba(0,0,0,0) 0%, rgba(0,0,0,0.20) 78%, rgba(0,0,0,0.38) 100%);z-index:2;pointer-events:none}}
  .anchor{{position:absolute;inset:0;background:linear-gradient(0deg,rgba(9,9,11,0.94) 0%,rgba(24,24,27,0.78) 18%,rgba(24,24,27,0.38) 32%,rgba(24,24,27,0) 46%),linear-gradient(125deg,rgba(24,24,27,0.94) 0%,rgba(24,24,27,0.82) 22%,rgba(24,24,27,0.45) 45%,rgba(24,24,27,0.12) 65%,rgba(24,24,27,0) 82%);z-index:3;pointer-events:none}}
  .shine{{position:absolute;inset:0;background:radial-gradient(ellipse 62% 50% at 22% 60%, rgba(245,158,11,0.58) 0%, rgba(245,158,11,0.30) 28%, rgba(245,158,11,0.10) 55%, rgba(245,158,11,0) 75%);z-index:4;pointer-events:none}}
  .grain{{position:absolute;inset:0;background-image:radial-gradient(rgba(251,191,36,0.06) 1px, transparent 1px);background-size:32px 32px;opacity:0.55;z-index:5;pointer-events:none}}

  .shield{{position:absolute;top:70px;right:70px;z-index:8;filter:drop-shadow(0 4px 20px rgba(0,0,0,0.55))}}
  .shield img{{height:66px;display:block}}
  .top-mast{{position:absolute;top:88px;left:90px;z-index:7;font-family:'Inter',sans-serif;font-size:11px;font-weight:700;letter-spacing:3.4px;text-transform:uppercase;color:rgba(250,250,249,0.7)}}
  .top-mast .dot{{color:var(--gold);margin:0 12px;opacity:0.8}}

  .content{{position:absolute;left:90px;bottom:130px;right:120px;z-index:7;max-width:760px}}
  .cat{{font-family:'Inter',sans-serif;font-size:12px;font-weight:700;letter-spacing:4px;text-transform:uppercase;color:var(--gold);margin-bottom:36px}}
  .cat::before{{content:'';display:inline-block;width:38px;height:1px;background:var(--gold);vertical-align:middle;margin-right:18px;margin-top:-4px}}
  h1{{font-family:'Playfair Display',serif;font-weight:700;font-size:{h1_size}px;line-height:0.94;letter-spacing:-2.6px;color:#fff;margin-bottom:0;text-shadow:0 2px 24px rgba(0,0,0,0.45)}}
  h1 .it,h1 .a{{font-style:italic;font-weight:700;color:var(--gold)}}
  h1 q{{quotes:'\\201C' '\\201D';font-style:italic;font-weight:700}}
  h1 em{{font-style:italic;font-weight:700;color:var(--gold)}}
  .rule{{width:48px;height:1px;background:var(--gold);margin:42px 0 30px;opacity:0.95}}
  .lede{{font-family:'Playfair Display',serif;font-style:italic;font-weight:400;font-size:25px;line-height:1.55;color:#fff;max-width:620px;letter-spacing:-0.2px;text-shadow:0 1px 14px rgba(0,0,0,0.65)}}
  .lede em{{font-style:normal;font-weight:700;color:var(--gold)}}
  .lede b{{color:#fff;font-weight:700;font-style:italic}}

  .footer{{position:absolute;bottom:54px;left:90px;right:90px;z-index:7;display:flex;justify-content:space-between;align-items:center}}
  .meta{{font-family:'Inter',sans-serif;font-size:11px;font-weight:700;letter-spacing:3.2px;text-transform:uppercase;color:rgba(250,250,249,0.8)}}
  .meta span{{color:var(--gold);margin:0 12px;opacity:0.7}}
  .url{{font-family:'Inter',sans-serif;font-size:11px;font-weight:700;letter-spacing:3.2px;text-transform:uppercase;color:rgba(250,250,249,0.8)}}
</style>
</head>
<body>
  <div class="photo"></div>
  <div class="vignette"></div>
  <div class="anchor"></div>
  <div class="shine"></div>
  <div class="grain"></div>
  <div class="shield"><img src="/impota-logo.png" alt="IMPOTA"></div>
  <div class="top-mast">IMPOTA <span class="dot">&middot;</span> Car Import Guide</div>
  <div class="content">
    <div class="cat">{cat}</div>
    <h1>{headline}</h1>
    <div class="rule"></div>
    <div class="lede">{lede}</div>
  </div>
  <div class="footer">
    <div class="meta">{meta}</div>
    <div class="url">impota.com</div>
  </div>
</body>
</html>
"""

POSTS = [
    {
        "file": "fb-56-where-cars-come-from.html",
        "title": "IMPOTA 56 — Where The Cars Come From",
        "cat": "The Source",
        "headline": 'Direct from<br><span class="it">Japanese auctions.</span>',
        "lede": "USS Tokyo. USS Yokohama. JU Aichi. Same auctions every dealer in Africa uses — <em>only without the markup.</em>",
        "meta": "USS · JU · TAA · 100+ auctions weekly",
        "bg": "pexels-tomfisk-3856433.jpg",
        "h1_size": 100,
    },
    {
        "file": "fb-57-the-journey.html",
        "title": "IMPOTA 57 — The Journey",
        "cat": "Auction To Driveway",
        "headline": '<span class="it">Tokyo.</span><br>Walvis Bay.<br><span class="it">Your driveway.</span>',
        "lede": "Five steps. Six weeks. <em>We walk you through every one.</em>",
        "meta": "Bid · Pay · Ship · Clear · Register",
        "bg": "pexels-tomfisk-5992514.jpg",
        "h1_size": 100,
    },
    {
        "file": "fb-58-who-its-for.html",
        "title": "IMPOTA 58 — Who It's For",
        "cat": "Built For First-Timers",
        "headline": 'Your first<br><span class="it">import.</span><br>Done right.',
        "lede": "If you've never bid on a Japanese auction, never seen a Bill of Lading, never paid NaTIS — <em>start here.</em>",
        "meta": "First-time importers · NA · ZA · BW · ZM",
        "bg": "pexels-mikhail-nilov-6964174.jpg",
        "h1_size": 110,
    },
    {
        "file": "fb-59-inside-impota.html",
        "title": "IMPOTA 59 — Inside IMPOTA",
        "cat": "What's Inside",
        "headline": 'Inside<br><span class="it">IMPOTA.</span>',
        "lede": "Landed-cost calculator. Auction-sheet decoder. 25+ vetted exporters. 20+ document templates. Country playbooks for <em>four countries.</em>",
        "meta": "Calculator · Decoder · Exporters · Docs · Playbooks",
        "bg": "pexels-n-voitkevich-7172786.jpg",
        "h1_size": 140,
    },
    {
        "file": "fb-60-calculator-shows.html",
        "title": "IMPOTA 60 — Calculator Shows",
        "cat": "Run The Numbers",
        "headline": 'CIF. Duty. VAT.<br><span class="it">Landed.</span>',
        "lede": "Type in the auction price. Pick your country. <em>See your total before you bid</em> — not after the dealer tells you.",
        "meta": "4 countries · Live duty rates · 60-second answer",
        "bg": "pexels-n-voitkevich-7172981.jpg",
        "h1_size": 105,
    },
    {
        "file": "fb-61-country-playbooks.html",
        "title": "IMPOTA 61 — Country Playbooks",
        "cat": "4 Countries · 4 Playbooks",
        "headline": '<span class="it">NA · ZA<br>BW · ZM.</span>',
        "lede": "Namibia's 12-year limit. South Africa's import permit. Botswana's age cap. Zambia's port options. <em>Every country, in detail.</em>",
        "meta": "Duties · Limits · Ports · Paperwork · Per country",
        "bg": "pexels-tomfisk-5972211.jpg",
        "h1_size": 130,
    },
    {
        "file": "fb-62-what-exporter-does.html",
        "title": "IMPOTA 62 — What An Exporter Does",
        "cat": "The Middleman, Explained",
        "headline": "Your exporter's<br><span class=\"it\">real job.</span>",
        "lede": "They bid at the auction, pay the seller, prep paperwork, deliver to port, book your container. <em>That's it.</em> Anything more is a markup.",
        "meta": "Bid · Pay · Paperwork · Port · Container",
        "bg": "pexels-mikhail-nilov-6964367.jpg",
        "h1_size": 110,
    },
    {
        "file": "fb-63-auction-grades.html",
        "title": "IMPOTA 63 — Auction Grades",
        "cat": "Reading The Sheet",
        "headline": '<span class="it">Grade 4.</span><br>Grade 3.5.<br><span class="it">Grade R.</span>',
        "lede": "Each tells you how much the car has been through. <em>Pay too much for the wrong grade — and you've lost before it ships.</em>",
        "meta": "Grade · Letter · Inspector notes · Damage codes",
        "bg": "pexels-bilal-ahmed-297238753-14020705.jpg",
        "h1_size": 110,
    },
    {
        "file": "fb-64-which-port.html",
        "title": "IMPOTA 64 — Pick Your Port",
        "cat": "Choose Your Port",
        "headline": 'Walvis Bay.<br>Durban.<br><span class="it">Dar es Salaam.</span>',
        "lede": "Three Africa-East ports. Different costs, different speeds, different paperwork. <em>We compare all three.</em>",
        "meta": "Walvis Bay · Durban · Dar es Salaam",
        "bg": "pexels-tomfisk-5982896.jpg",
        "h1_size": 100,
    },
    {
        "file": "fb-65-documents.html",
        "title": "IMPOTA 65 — Documents You'll Need",
        "cat": "The Paperwork",
        "headline": 'BL. SAD500.<br><span class="it">NaTIS. Police.</span>',
        "lede": "Bill of Lading. Customs declaration. Vehicle registration. Police clearance. <em>Every form, every signature, in order.</em>",
        "meta": "20+ document templates · All countries · Inside",
        "bg": "pexels-towfiqu-barbhuiya-3440682-12001949.jpg",
        "h1_size": 110,
    },
]

count = 0
for p in POSTS:
    out = ROOT / p["file"]
    out.write_text(TEMPLATE.format(**p), encoding="utf-8")
    print(f"  OK  {p['file']}")
    count += 1

print(f"\nGenerated {count} explainer posts in {ROOT}")
